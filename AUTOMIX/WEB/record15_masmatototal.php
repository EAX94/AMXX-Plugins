	...
	<body>
		<?php
			...
			echo "<tr align='center' bgcolor='#52697B'>
					...
					<th width='15%' align='center'>Matados</th>
				</tr>";
			
			$topQuery = "SELECT name, SUM(score) AS total
						FROM(SELECT `w_pj_1` AS name, `w_pj_1_k` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_2` AS name, `w_pj_2_k` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_3` AS name, `w_pj_3_k` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_4` AS name, `w_pj_4_k` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_5` AS name, `w_pj_5_k` AS score FROM amix_partidos) A
						GROUP BY name
						ORDER BY total DESC
						LIMIT 15;";
