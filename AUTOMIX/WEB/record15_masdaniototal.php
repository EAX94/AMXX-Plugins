	...
	<body>
		<?php
			...
			$topQuery = "SELECT name, SUM(score) AS total
						FROM(SELECT `w_pj_1` AS name, `w_pj_1_dmg` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_2` AS name, `w_pj_2_dmg` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_3` AS name, `w_pj_3_dmg` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_4` AS name, `w_pj_4_dmg` AS score FROM amix_partidos UNION ALL
						SELECT `w_pj_5` AS name, `w_pj_5_dmg` AS score FROM amix_partidos) A
						GROUP BY name
						ORDER BY total DESC
						LIMIT 15;";
