...
public plugin_init()
{
	...
	kz_uq_script_work = register_cvar("kz_uq_script_work", "0");
	...
	register_concmd("amx_reset_uqtops","reset_tops",ADMIN_LEVEL_A ,"reset all tops");
...
public plugin_cfg()
{
	...
	if(plugin_id==-1)
	{
		server_print("[KZ] No se encontro el uq_jumpstats_tops.amxx");
	}
	else
	{
...
stock kz_make_cvarexec(const config[])
{
	...
	fprintf(f, "// say /height - on/off display height^n");
	...
	fprintf(f, "// say /jheight - on/off showing jump height^n");
...
public Log_script(f_frames,cheated_frames,id,Float:log_dist,Float:log_max,Float:log_pre,log_str,log_sync,jump_type_str[],wpn_str[],punishments[],t_str[40*NSTRAFES])
{
	...
	if(uq_script_notify)
	{
		ColorChat(0,RED,"^x01[%s]^x04 %s^x03 Probablemente uso el script^x01 %s^x03!!!",prefix,username,jump_type_str);
	}
}
public krasnota(id)
{                
...
public DoSpeed(taskid)
{
	...
	if( (alive || spectatedplayer > 0))
	{
		...
		if(speedtype[taskid])
		{
			...
			show_dhudmessage(taskid, "%d unidades/segundos", floatround(speedy, floatround_floor));
		}
		else
		{
			set_hudmessage(speed_r, speed_g, speed_b, speed_x, speed_y, 0, 0.0, 0.2, 0.0, 0.0, h_speed);
			show_hudmessage(taskid, "%d unidades/segundos", floatround(speedy, floatround_floor));		
		}
	}
}
public wait(id)
...
public client_command(id)
{
	...
	if(uq_info)
	{
		...
		else if(record_start[id]==1 && equali( command, "specmode" ))
		{
			...
			//show_hudmessage(id, "Server using uq_jumpstats by BorJomi v%s",VERSION);
			...
			if( g_alive[id] && team[id]==uq_team)
			{
				...
				if(strafe_num[id]>NSTRAFES1)
				{
					...
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						if( (i == id || is_spec_user[i]))
						{
							...
							show_hudmessage(i,"Has excedido el numero maximo de strafes: ^"%d^" ^n El tuyo ^"%d^")",NSTRAFES1,strafe_num[id]);	
						}
					}
					return FMRES_IGNORED;
				}
				...
				if((button&IN_RIGHT || button&IN_LEFT) && !(flags&FL_ONGROUND))
				{
					for(new i = INFO_ONE; i < max_players; i++ )
					{
						if( (i == id || is_spec_user[i]))
						{
							client_print(i,print_center,"Utilizas script o las flechas dercha/izquierda");
							JumpReset(id,28);
							return FMRES_IGNORED;
						}
					}
						...
						{	
							if(height_show[i]==true )
							{
								if(heightland_origin-heightoff_origin[id]==0.0)
								{
									...
									show_hudmessage(i, "Salto realizado a la misma altura");
								}
								else if(heightland_origin-heightoff_origin[id]>0.0)
								{
									set_hudmessage(prest_r,prest_g, prest_b, stats_x, stats_y, 0, 0.0, 0.7, 0.1, 0.1, h_stats);
									show_hudmessage(i, "Saltado: %.03f",heightland_origin-heightoff_origin[id]);
								}
								else if(heightland_origin-heightoff_origin[id]<0.0)
								{
									set_hudmessage(prest_r,prest_g, prest_b, stats_x, stats_y, 0, 0.0, 0.7, 0.1, 0.1, h_stats);
									show_hudmessage(i, "Caido: %.03f",floatabs(heightland_origin-heightoff_origin[id]));
								}
							}
						}
					}
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						if( (i == id || is_spec_user[i]))
						{
							if(height_show[i]==true )
							{
								if(heightland_origin-heightoff_origin[id]==0.0)
								{
									client_print( i, print_console,"Salto realizado a la misma altura");
								}
								else if(heightland_origin-heightoff_origin[id]>0.0)
								{
									client_print( i, print_console,"Saltado: %.03f",heightland_origin-heightoff_origin[id]);
								}
								else if(heightland_origin-heightoff_origin[id]<0.0)
								{
									client_print( i, print_console,"Caido: %.03f",floatabs(heightland_origin-heightoff_origin[id]));
								}
							}
						}
					}
				...
				if(flags&FL_ONGROUND && slidim[id]==true && Pmaxspeed == 250.0)
				{
					...
					{ 
						...
						if(gHasColorChat[ids] ==true || ids==id)
						{
							if(needslide[id]-groundslide[id]==slidedist[id])
							{
								ColorChat(ids, GREY, "[%s] %s salto %d.xxx unidades con Slide lj!^x01%s",prefix, g_playername[id], floatround(slidedist[id], floatround_floor),airacel[id]);
							}
							else 
							ColorChat(ids, GREY, "[%s] %s salto %.3f unidades with con lj!^x01%s",prefix, g_playername[id], slidedist[id],airacel[id]);				
						}
					}
						...
						for( new i = INFO_ONE; i < max_players; i++ )
						{
							...
							{	
								if(jheight_show[i]==true )
								{
									...
									show_hudmessage(i, "Altura del salto: %.03f",jheight[id]);
									
									client_print( i, print_console,"Altura del salto: %.03f",jheight[id]);
								}
							}
						}
								...
								{	
									if(showpre[i]==true && prestrafe[id]>min_prestrafe[id])
									{
										...
										{	
											...
											show_hudmessage(i, "Tu prestrafe %.01f es demasiado alto (%.01f)",prestrafe[id],Pmaxspeed * 1.2);
										}
									}
								}
							}
				...
				{	
					...
					if (velocity[2] == -240.0)
					{
						...
						formatex(Jtype[id],32,"WeirdJump antes de dd");
					...
					else 
					{
						...
						for( i = INFO_ONE; i < max_players; i++ )
						{
							if( (i == id || is_spec_user[i]))
							{
									...
									if(prestrafe[id]>min_prestrafe[id])
									{
										if(jump_type[id] == Type_Double_CountJump && showpre[id]==true && uq_dcj==1)
										{
											if(CjafterJump[id]==2)
											{
												...
												if(ddforcjafterbhop[id])
													show_hudmessage(i, "DCJ luego de Bhop Pre: %.03f",prestrafe[id]);	
												else if(ddforcjafterladder[id])
													show_hudmessage(i, "DCJ luego de Ladder Pre: %.03f",prestrafe[id]);
												else
													show_hudmessage(i, "DCJ luego de Jump Pre: %.03f",prestrafe[id]);		
											}
											else
											{
												set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
										...
										else if(jump_type[id] == Type_CountJump && showpre[id]==true && uq_cj==1)
										{
											if(CjafterJump[id]==1)
											{
												...
												if(ddforcjafterbhop[id])
													show_hudmessage(i, "CJ luego de Bhop Pre: %.03f",prestrafe[id]);	
												else if(ddforcjafterladder[id])
													show_hudmessage(i, "DCJ luego de Ladder Pre: %.03f",prestrafe[id]);
												else
													show_hudmessage(i, "CJ luego de Jump Pre: %.03f",prestrafe[id]);
											}
											else
											{
												set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
										...
										else if(jump_type[id] == Type_Multi_CountJump  && showpre[id]==true  && uq_mcj==1)
										{
											if(CjafterJump[id]==3)
											{
												...
												if(ddforcjafterbhop[id])
													show_hudmessage(i, "MCJ luego de Bhop Pre: %.03f",prestrafe[id]);	
												else if(ddforcjafterladder[id])
													show_hudmessage(i, "DCJ luego de Ladder Pre: %.03f",prestrafe[id]);
												else
													show_hudmessage(i, "MCJ luego de Jump Pre: %.03f",prestrafe[id]);	
											}
											else
											{
												set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
										...
										else if((jump_type[id]==Type_Drop_CountJump)&& showpre[id]==true)
										{
											...
											{
												set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
												show_hudmessage(i, "Doble Drop CJ Pre: %.03f",prestrafe[id]);
											}
										...
										else if((jump_type[id]==Type_StandUp_CountJump) && showpre[id]==true && uq_drsbj==1)
										{
											...
											else if(ddstandcj[id])
											{
												...
												{
													set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
													show_hudmessage(i, "Doble SCJ After Jump Pre: %.03f",prestrafe[id]);
												}
											...
											else
											{
												...
												{
													set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
													show_hudmessage(i, "Doble SCJ Pre: %.03f",prestrafe[id]);
												}
								...
								{
									set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 1.5, 0.1, 0.1, h_prest);
									show_hudmessage(i, "Tu prestrafe %.03f es demasiado alto (%.01f)",prestrafe[id],Pmaxspeed * 1.2);
								}
							}
						}
					}
					...
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						...
						{	
							if(showpre[i]==true && multibhoppre[id] && speed[id]>50.0)
							{
								...
								{
									set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
									show_hudmessage(i, "Multi Bhop Pre: %.03f",speed[id]);
								}
								else
								{	if((uq_bj==1 || uq_sbj==1))
									{
										set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 1.5, 0.1, 0.1, h_prest);
										show_hudmessage(i, "Tu prestrafe %.01f es alto(%.01f)",prestrafe[id],Pmaxspeed * 1.2);
									}
								}
							}
						}
						...
						for( i = INFO_ONE; i < max_players; i++ )
						{
							...
							{	
								if(showpre[id]==true && failearly[id]==true && (uq_bj==1 || uq_sbj==1))
								{
									...
									show_hudmessage(id, "Presionaste para saltar antes de tiempo");
								}
							}
						}
					}
				...
				{	
					...
					if(prestrafe[id]>200 && jump_type[id]==Type_DuckBhop)
					{
						...
						set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 1.5, 0.1, 0.1, h_prest);
						show_hudmessage(id, "Tu prestrafe %.01f probablemente este bugeado por DuckBhop",prestrafe[id]);
					}
					new summad,summws;
					for(new i=0;i<NSTRAFES;i++)
					{
					...
					if(strafe_num[id] < NSTRAFES)
					{
						...
						if(strafe_num[id]!=0)
						{
							...
							{	
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^t^tVelocidad de giro: %4.3f",strafe_stat_speed[id][0][0]);
							}
							if (duckstring[id]==false)
							{							
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^t^tSin agacharse");
							}
							...
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tArma: %s",weapon_name);
							}
							...
							if(Show_edge[id] && failed_jump[id]==false && jump_type[id]!=Type_ladder && jumpblock[id]<user_block[id][0] && jumpblock[id]>user_block[id][1] && edgedist[id]<100.0 && edgedist[id]!=0.0 && (jumpblock[id]+edgedist[id])<distance[id])
							{
								...
								{
									strLen =strLen+format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tBloque -- %d^n^tJumpoff -- %f",jumpblock[id],edgedist[id]);
								}
								else if(land_bug)
								{
									strLen =strLen+format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tBloque -- %d^n^tJumpoff -- %f^n^tLanding -- %f(bg)",jumpblock[id],edgedist[id],Landing);
								}
								else strLen =strLen+format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tBloque -- %d^n^tJumpoff -- %f^n^tLanding -- %f",jumpblock[id],edgedist[id],Landing);
							}
							...
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tAltura: %0.3f",upheight[id]);
							}
							if(fps_hight[id] && jump_type[id]!=Type_ladder)
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tTenes mas de 101 fps o lag");
							}
							if(ladderbug[id])
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tProbablemente bugeaste una escalera");
								failed_jump[id]=true;
							}
							if(find_ladder[id] && jump_type[id]==Type_DuckBhop)
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tDuckBhop no funciona cerca de una escalera");
								failed_jump[id]=true;
							}
							if(touch_somthing[id])
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tProbablemente tocaste algo");
							}
							if(find_script)
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "^n^n^tProbablemente usas script");
								failed_jump[id]=true;
							}
							if(trigger_protection[id])
							{
								strLen += format(strMess[strLen],(40*NSTRAFES)-strLen-1, "Lo siento pero el primer salto luego de un desencadenante esta bloqueado");
							}
						}
					}
					//client_print(0,print_chat,"angles=%d",angles_arry[id]);	
					...
					switch( jump_type[id] )
					{
						...
						case 2: 
						{
							if(CjafterJump[id]==1)
							{
								if(ddforcjafterbhop[id])
								{
									formatex( type, 32, "CJ luego de Bhop" );
									
									formatex(Jtype[id],32,"CountJump luego de Bhop");
									formatex(Jtype1[id],32,"cjab");
								}
								else if(ddforcjafterladder[id])
								{
									formatex( type, 32, "CJ luego de Ladder" );
									
									formatex(Jtype[id],32,"CountJump luego de Ladder");
									formatex(Jtype1[id],32,"cjal");
								}
								else
								{
									formatex( type, 32, "CJ luego de Jump" );
									
									formatex(Jtype[id],32,"CountJump luego de Jump");
						...
						case 9:
						{
							if(CjafterJump[id]==2)
							{
								if(ddforcjafterbhop[id])
								{
									formatex( type, 32, "DCJ luego de Bhop" );
									
									formatex(Jtype[id],32,"Doble CountJump luego de Bhop");
									formatex(Jtype1[id],32,"dcjab");
								}
								else if(ddforcjafterladder[id])
								{
									formatex( type, 32, "DCJ luego de Ladder" );
									
									formatex(Jtype[id],32,"Doble CountJump luego de Ladder");
									formatex(Jtype1[id],32,"dcjal");
								}
								else
								{
									formatex( type, 32, "DCJ luego de Jump" );
									
									formatex(Jtype[id],32,"Double CountJump luego de Jump");
						...
						case 11: 
						{
							if(CjafterJump[id]==3)
							{
								if(ddforcjafterbhop[id])
								{
									formatex( type, 32, "MCJ luego de Bhop" );
									
									formatex(Jtype[id],32,"Multi luego de Bhop");
									formatex(Jtype1[id],32,"mcjab");
								}
								else if(ddforcjafterladder[id])
								{
									formatex( type, 32, "MCJ luego de Ladder" );
									
									formatex(Jtype[id],32,"Multi luego de Ladder");
									formatex(Jtype1[id],32,"mcjal");
								}
								else
								{
									formatex( type, 32, "MCJ luego de Jump" );
									
									formatex(Jtype[id],32,"Multi luego de Jump");
						...
						{	
							...
							{
								add( type, 32, "Doble Drop CJ", 25 );
								formatex(Jtype[id],32,"Doble Drop CJ");
							}
						...
						{	if(dropaem[id])
							...
							else if(ddstandcj[id])
							{
								...
								{
									add( type, 32, "Doble SCJ After Jump", 25 );
									formatex(Jtype[id],32,"Doble StandUp CountJump After Jump");
							...
							else
							{
								...
								{
									add( type, 32, "Doble SCJ", 25 );
									formatex(Jtype[id],32,"Doble StandUp CountJump");
					...
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						...
						{	
							...
							{
								set_hudmessage(stats_r, stats_g, stats_b, stats_x, stats_y, 0, 6.0, 2.5, 0.1, 0.3, h_stats  );	
								show_hudmessage( i, "Distancia %s: %f^nVel. Max.: %f (%.03f)^nPrestrafe: %f^nStrafes: %d^nSync: %d%%^nBhop Jump's: %d", type, distance[id], maxspeed[id], gain[id], prestrafe[id],strafe_num[id], sync_[id],bhop_num[id]);
							}
							...
							{
								set_hudmessage(stats_r, stats_g, stats_b, stats_x, stats_y, 0, 6.0, 2.5, 0.1, 0.3, h_stats );	
								show_hudmessage( i, "Distancia %s: %f^nVel. Max.: %f (%.03f)^nPrestrafe: (%.01f) (%.01f) %f^nStrafes: %d^nSync: %d%%^nPrestrafe Sync: %d%%", type, distance[id], maxspeed[id], gain[id], prest1[id],prest[id], prestrafe[id],strafe_num[id], sync_[id],dd_sync[id]);
							}
							...
							{
								set_hudmessage(stats_r, stats_g, stats_b, stats_x, stats_y, 0, 6.0, 2.5, 0.1, 0.3, h_stats  );	
								show_hudmessage( i, "Distancia %s: %f^nVel. Max.: %f (%.03f)^nPrestrafe: %f^nStrafes: %d^nSync: %d%%^n", type, distance[id], maxspeed[id], gain[id], prestrafe[id],strafe_num[id], sync_[id]);
							}
							...
							{
								set_hudmessage(stats_r, stats_g, stats_b, stats_x, stats_y, 0, 6.0, 2.5, 0.1, 0.3, h_stats);
								show_hudmessage( i, "Distancia %s: %f^nVel. Max.: %f (%.03f)^nPrestrafe: (%.03f) %f^nStrafes: %d^nSync: %d%%^nPrestrafe Sync: %d%%", type, distance[id], maxspeed[id], gain[id], prest1[id],prestrafe[id],strafe_num[id], sync_[id],dd_sync[id]);
							}
							...
							{
								set_hudmessage(stats_r, stats_g, stats_b, stats_x, stats_y, 0, 6.0, 2.5, 0.1, 0.3, h_stats );
								show_hudmessage( i, "Distancia %s: %f^nVel. Max.: %f (%.03f)^nPrestrafe: (%.03f) %f^nDucks: %d%^nStrafes: %d^nSync: %d%%^nPrestrafe Sync: %d%%", type, distance[id], maxspeed[id], gain[id], prest1[id],prestrafe[id],ducks[id], strafe_num[id], sync_[id],dd_sync[id]);
							}
					...
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						if( (i == id || is_spec_user[i]) && g_lj_stats[i]==true)
						{
							...
							if((jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_Drop_BhopLongJump || jump_type[id]==Type_WeirdLongJump || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump) )
							{
								...
								client_print( i, print_console, "Distancia %s: %f Vel. Max.: %f (%.03f) Prestrafe: %f Strafes: %d Sync: %d", type, distance[id], maxspeed[id], gain[id], prestrafe[id], strafe_num[id],sync_[id] );
							}
							else if(jump_type[id]==Type_Multi_Bhop )
							{
								client_print( i, print_console, " ");
								client_print( i, print_console, "Distancia %s: %f Vel. Max.: %f (%.03f) Prestrafe: %f Strafes: %d Sync: %d Bhop Jump's: %d", type, distance[id], maxspeed[id], gain[id], prestrafe[id], strafe_num[id],sync_[id] ,bhop_num[id]);
							...
							{
								client_print( i, print_console, " ");
								client_print( i, print_console, "Distancia %s: %f Vel. Max.: %f (%.03f) Prestrafe: (%.03f) %f Strafes: %d Sync: %d Prestrafe Sync: %d", type, distance[id], maxspeed[id], gain[id], prest1[id], prestrafe[id], strafe_num[id],sync_[id],dd_sync[id] );
							}
							...
							{
								client_print( i, print_console, " ");
								client_print( i, print_console, "Distancia %s: %f Vel. Max.: %f (%.03f) Prestrafe: (%.01f) (%.01f) %f Strafes: %d Sync: %d Prestrafe Sync: %d", type, distance[id], maxspeed[id], gain[id], prest1[id],prest[id], prestrafe[id], strafe_num[id],sync_[id],dd_sync[id] );
							}
							...
							{
								client_print( i, print_console, " ");
								client_print( i, print_console, "Distancia %s: %f Vel. Max.: %f (%.03f) Prestrafe: (%.03f) %f Ducks: %d% Strafes: %d Sync: %d Prestrafe Sync: %d", type, distance[id], maxspeed[id], gain[id], prest1[id],prestrafe[id],ducks[id],strafe_num[id], sync_[id],dd_sync[id]);
							}
							...
							if(jump_type[id]==Type_ladder && strafe_stat_speed[id][0][0]!=0)
							{
								client_print(i, print_console, "Velocidad de giro: %.03f", strafe_stat_speed[id][0][0]);
							}
							else if(jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id] == Type_Drop_CountJump))
							{
								client_print( i, print_console, "..................................................");
								client_print( i, print_console, "Prestrafe antes de agacharte: %.03f",prest1[id]);
								for(new ss=2;ss<=ducks[id];ss++)
								client_print( i, print_console, "Prestrafe despues #%d agachadas: %.03f",ss-1,statsduckspeed[id][ss]);
								client_print( i, print_console, "Prestrafe antes de saltar: %.03f",prestrafe[id]);
								client_print( i, print_console, "..................................................");
							}
							else if(jump_type[id]==Type_Double_CountJump || (multiscj[id]==1 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==1 && jump_type[id] == Type_Drop_CountJump))
							{
								client_print( i, print_console, "..................................................");
								client_print( i, print_console, "Prestrafe antes de agacharte: %.03f",prest1[id]);
								client_print( i, print_console, "Prestrafe despues de la primer agachada: %.03f",prest[id]);
								client_print( i, print_console, "Prestrafe antes de saltar: %.03f",prestrafe[id]);
								client_print( i, print_console, "..................................................");
							}
							else if(jump_type[id]==Type_Up_Bhop || jump_type[id]==Type_Up_Stand_Bhop || jump_type[id]==Type_Up_Bhop_In_Duck)
							{
								client_print( i, print_console,"Altura: %0.3f",upheight[id]);
							}
							if(wpn!=29 && wpn!=17 && wpn!=16)
							{
								client_print( i, print_console,"Arma: %s",weapon_name);
							}
							if(fps_hight[id] && jump_type[id]!=Type_ladder)
							{
								client_print( i, print_console,"Tenes mas de 101 fps o lag");
								fps_hight[id]=false;
							}
							if(ladderbug[id])
							{
								client_print( i, print_console,"Probablemente bugeaste una escalera");
								ladderbug[id]=false;
							}
							if(find_ladder[id] && jump_type[id]==Type_DuckBhop)
							{
								client_print( i, print_console,"DuckBhop no funciona cerca de una escalera");
								find_ladder[id]=false;
							}
							if(touch_somthing[id])
							{
								client_print( i, print_console,"Probablemente tocaste algo");
							}
							if(Show_edge[id] && failed_jump[id]==false && jump_type[id]!=Type_ladder && jumpblock[id]<user_block[id][0] && jumpblock[id]>user_block[id][1] && edgedist[id]<100.0 && edgedist[id]!=0.0 && (jumpblock[id]+edgedist[id])<distance[id])
							{
								if((jumpblock[id]+Landing+edgedist[id])>(distance[id]+10.0) || Landing<=0.0)
								{
									client_print( i, print_console,"Bloque -- %d! Jumpoff -- %f!",jumpblock[id],edgedist[id]);
								}
								else if(land_bug )
								{
									client_print( i, print_console,"Bloque -- %d! Jumpoff -- %f! Landing -- %f(bg)!",jumpblock[id],edgedist[id],Landing);
								}
								else client_print( i, print_console,"Bloque -- %d! Jumpoff -- %f! Landing -- %f!",jumpblock[id],edgedist[id],Landing);
							}
							...
							if(find_script)
							{
								client_print( i, print_console,"Probablemente usas script");
								
							}
							if(trigger_protection[id])
							{
								client_print( i, print_console,"Lo siento pero el primer salto luego de un desencadenante esta bloqueado");
					...
					{
						new punishment[64];
						formatex(punishment,63,"Stats Blocked");
						
						switch(uq_script_punishment)
						{
							case 1:
							{
								formatex(punishment,63,"Kicked");
							}
							case 2:
							{
								formatex(punishment,63,"Banned for %d minutes",uq_ban_minutes);
							}
						}
							...
							if( !failed_jump[id] )
							{
										     ...
										     || jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										ColorChat(ids, RED, "[%s] %s salto %.3f unidades con %s!^x01%s ^x01%s ^x03%s",prefix, g_playername[id], distance[id],Jtype[id],weapon_name,pre_type[id],airacel[id]);
									}
										...
										ColorChat(ids, BLUE, "[%s] %s salto %.3f unidades con %s!^x01%s ^x01%s ^x03%s",prefix, g_playername[id], distance[id],Jtype[id],weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/perfect");
									
										ColorChat(ids, GREEN, "[%s] %s salto %.3f unidades con %s!^x01%s ^x01%s ^x04%s",prefix, g_playername[id], distance[id],Jtype[id],weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >=good_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/impressive");
									
										ColorChat(ids, GREY, "[%s] %s salto %.3f unidades con %s!^x01%s ^x01%s ^x03%s",prefix, g_playername[id], distance[id],Jtype[id],weapon_name,pre_type[id],airacel[id]);
									}
								}
								else if(jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id]==Type_Drop_CountJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										if( uq_light ) krasnota(id);
										ColorChat(ids, RED, "[%s] %s salto %.3f unidades con %s! (%d ducks)^x01%s ^x01%s ^x03%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= leet_dist  ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/mod_wickedsick");
										...
										if( uq_light ) sineva(id);
										ColorChat(ids, BLUE, "[%s] %s salto %.3f unidades con %s! (%d ducks)^x01%s ^x01%s ^x03%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/perfect");
									
										ColorChat(ids, GREEN, "[%s] %s salto %.3f unidades con %s! (%d ducks)^x01%s ^x01%s ^x04%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >=good_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/impressive");
									
										ColorChat(ids, GREY, "[%s] %s salto %.3f unidades con %s! (%d ducks)^x01%s ^x01%s ^x03%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],weapon_name,pre_type[id],airacel[id]);
									}
								}
							}	
						}
					...
					if( get_gametime( ) - duckoff_time[id] < 0.3 )
					{
						...
						for( new i = INFO_ONE; i < max_players; i++ )
						{
							if( (i == id || is_spec_user[i]))
							{
								...
								{
									set_hudmessage(prest_r,prest_g, prest_b, prest_x, prest_y, 0, 0.0, 0.7, 0.1, 0.1, h_prest);
									show_hudmessage(i, "Duck Pre: %.03f",speed[i]);
								}
							}
						}
					}
					...
					{	
						...
						if( started_cj_pre[id] )
						{
							...
							if(jump_type[id] != Type_Drop_CountJump)
							{
								...
								{
									formatex(Jtype[id],32,"CountJump despues de saltar");
									formatex(Jtype1[id],32,"cjaj");
						...
						else if( started_multicj_pre[id] )
						{
							...
							if(jump_type[id] != Type_Drop_CountJump)
							{
								...
								if(nextbhop[id] || bhopaem[id])
								{
									formatex(Jtype[id],32,"Doble CountJump despues de saltar");
									formatex(Jtype1[id],32,"dcjaj");
								...
								{
									formatex(Jtype[id],32,"Doble CountJump");
						...
						{	
							if(jump_type[id] != Type_Drop_CountJump)
							{
								...
								if(nextbhop[id] || bhopaem[id])
								{
									formatex(Jtype[id],32,"Multi CountJump despues de saltar");
									formatex(Jtype1[id],32,"mcjaj");
...
{
	new szReason[64];
	formatex(szReason,63,"Has sido expulsado por utilizar un script %s",j_type_str);
...
{	
	...
	for (new i = 0; i < NTOP; i++)
	{
		if( d > tmp_distance[i] )
		{
			...
			client_print(0,print_chat,"[%s] %s is now %i in %s top with %f ",prefix,g_playername[id],(i+1),type,dd);				
...
{	
	...
	for (new i = 0; i < NTOP; i++)
	{
		if( d > tmp_distance[i] )
		{
			...
			client_print(0,print_chat,"[%s] %s ahora esta %i en el top del mapa %s con %f ",prefix,g_playername[id],(i+1),type,dd);				
	...
	for (new i = 0; i < NTOP; i++)
	{
		if( bb >= tmp_block[i] && d>tmp_dist)
		{
			...
			{	
				...
				{
					client_print(0,print_chat,"[%s] %s esta ahora %i en el top de bloques de %s con %d",prefix,g_playername[id],(j+1),type,bb);				
				}
			}
			if((i+1)<11 && uq_screen==1)
			{
	...
	for (new i = 0; i < NTOP; i++)
	{
		if( bb >= tmp_block[i] && d>tmp_dist)
		{
			...
			{	
				if(d==tmp_distance[j] && equali(tmp_ip[j],rb))
				{
					client_print(0,print_chat,"[%s] %s esta ahora %i en el top de bloques con arma %s (velocidad del arma: %d) con %d",prefix,g_playername[id],(j+1),type,pev_max_speed,bb);				
				}
			}
			if((i+1)<11 && uq_screen==1)
			{
...
{	
	...
	for (new i = 0; i < NTOP; i++)
	{
		if( d > tmp_distance[i] )
		{
			...
			client_print(0,print_chat,"[%s] %s esta ahora %i en el top de %s (velocidad del arma: %d) con %f ",prefix,g_playername[id],(i+1),type,pev_max_speed,dd);				
	...
	for (new i = 0; i < NTOP; i++)
	{
		if( d > map_dist[i] )
		{
			...
			{ 
				...
				{	
					client_print(ids,print_chat,"[%s] %s esta ahora %i en el top del mapa con %.03f estilo %s",prefix,g_playername[id],(i+1),dd,typ);	
				}
			}
			break;
		}
...
{	
	if( !gHasColorChat[id] )
	{
		...
		ColorChat(id, GREEN, "^x04[%s]^x01 ColorChat habilitado. Para deshabilitarlo escribi /colorchat.", prefix);
	}
	else 
	{
		gHasColorChat[id] = false;
		ColorChat(id, GREEN, "^x04[%s]^x01 ColorChat deshabilitado. Para habilitarlo escribi /colorchat.", prefix);
	}
...
public cmdljStats( id ) {
	...
	{
		ColorChat(id, RED, "^x04[%s]^x01 Estadisticas:^x03 deshabilitadas",prefix);
		g_lj_stats[id]=false;
	...
	else 
	{
		...
		g_lj_stats[id]=true;
		ColorChat(id, BLUE, "^x04[%s]^x01 Estadisticas:^x03 habilitadas",prefix);
	}
	return PLUGIN_HANDLED;
}
...
{		
	ColorChat(id, GREY, "^x01[%s]^x04 Plugin:^x01 Jump Stats^x04 by^x03 BorJomi ^x04Version:^x01 %s",prefix,VERSION);
	if(kz_sql==1)
	{
		ColorChat(id, GREY, "^x01[%s]^x04 PHP^x01 and^x04 SQL^x01 by^x03 Light",prefix);	
		ColorChat(id, BLUE, "^x01[%s]^x04 Visit^x03 http://unique-kz.com^x01 and^x03 http://epic-s.ru",prefix);
	}
	else ColorChat(id, BLUE, "^x01[%s]^x04 Visit^x03 http://unique-kz.com",prefix);
}*/
public pre_stats(id)
{	
	if(kz_stats_pre[id]==true) 
	{
		ColorChat(id, RED, "^x04[%s]^x01 Estadisticas de prestrafe:^x03 deshabilitadas",prefix);
		kz_stats_pre[id]=false;
	}
	else 
	{
		kz_stats_pre[id]=true;
		ColorChat(id, BLUE, "^x04[%s]^x01 Estadisticas de prestrafe:^x03 habilitadas",prefix);
	}
	return PLUGIN_HANDLED;
}
...
{	
	...
	{
		ColorChat(id, RED, "^x04[%s]^x01 Estadisticas de Strafe's:^x03 deshabilitadas",prefix);
		streifstat[id]=false;
	}
	else 
	{
		streifstat[id]=true;
		ColorChat(id, BLUE, "^x04[%s]^x01 Estadisticas de Strafe's:^x03 habilitadas",prefix);
	}
	return PLUGIN_HANDLED;
}
public cmdljbeam(id)
{
	if(ingame_strafe[id])
	{
		ColorChat(id, RED, "^x04[%s]^x01 El rayo^x03 no funciona^x01 con el /istrafe",prefix);
	}
	else
	{
		if(kz_beam[id]==true) 
		{
			ColorChat(id, RED, "^x04[%s]^x01 Rayo:^x03 deshabilitado",prefix);
			kz_beam[id]=false;
		}
		else 
		{
			kz_beam[id]=true;
			ColorChat(id, BLUE, "^x04[%s]^x01 Rayo:^x03 habilitado",prefix);
		}
	}
	return PLUGIN_HANDLED;
}
public show_pre(id)
{
	if(showpre[id]==true) 
	{
		ColorChat(id, RED, "^x04[%s]^x01 ShowPre:^x03 deshabilitado",prefix);
		showpre[id]=false;
	}
	else 
	{
		showpre[id]=true;
		ColorChat(id, BLUE, "^x04[%s]^x01 ShowPre:^x03 habilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public show_speed(id)
{
	...
	if(uq_admins==1 && !player_admin[id])
	{
		ColorChat(id, RED, "^x04[%s]^x01 Speed:^x03 deshabilitado -^x04 Tu no eres admin",prefix);
	}
	else if(g_team!=uq_team)
	{
		ColorChat(id, RED, "^x04[%s]^x01 Speed^x03 deshabilitado^x01 para tu equipo",prefix);	
	}
	else
	{
		if(jofon[id])
		{
			ColorChat(id,RED,"^x04[%s]^x03 Necesitas apagar /joftr",prefix);
		}
		else
		{
			if(speedon[id]==false) 
			{
				ColorChat(id, BLUE, "^x04[%s]^x01 Speed:^x03 habilitado",prefix);
				speedon[id]=true;
			...
			else 
			{
				...
				ColorChat(id, RED, "^x04[%s]^x01 Speed:^x03 deshabilitado",prefix);
			}
		}
	}
	return PLUGIN_HANDLED;
...
public trainer_jof(id)
{
	...
	if(uq_admins==1 && !player_admin[id])
	{
		ColorChat(id, RED, "^x04[%s]^x01 Indicador JumpOff:^x03 deshabilitado -^x04 No sos admin",prefix);
	}
	else if(g_team!=uq_team)
	{
		ColorChat(id, RED, "^x04[%s]^x01 Indicador JumpOff:^x03 deshabilitado^x01 para tu equipo",prefix);	
	}
	else
	{
		if(speedon[id])
		{
			ColorChat(id,RED,"^x04[%s]^x03 Necesitas apagar /speed",prefix);
		}
		else
		{
			if(jofon[id]==false) 
			{
				ColorChat(id, BLUE, "^x04[%s]^x01 Indicador JumpOff:^x03 habilitado",prefix);
				jofon[id]=true;
				jof[id]=0.0;
			...
			else 
			{
				...
				ColorChat(id, RED, "^x04[%s]^x01 Indicador JumpOff:^x03 deshabilitado",prefix);
			}
		}
	}
	return PLUGIN_HANDLED;
...
public speed_type(id)
{
	...
	{
		ColorChat(id,BLUE,"^x04[%s]^x01 Speed:^x03 Grande", prefix);
		speedtype[id]=true;
	}
	else 
	{
		speedtype[id]=false;
		ColorChat(id,RED,"^x04[%s]^x01 Speed:^x03 Chico", prefix);
	}
}
public show_jheight(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Mostrar altura del salto:^x03 habilitado",prefix);
		jheight_show[id]=true;
	}
	else 
	{
		jheight_show[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Mostrar altura del salto:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public show_jof(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Mostrar JumpOff:^x03 habilitado",prefix);
		showjofon[id]=true;
	}
	else 
	{
		showjofon[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Mostrar JumpOff:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public show_early(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Aviso de bhop fallido:^x03 habilitado",prefix);
		failearly[id]=true;
	}
	else 
	{
		failearly[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Aviso de bhop fallido:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public multi_bhop(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Multi Bhop Prestrafe:^x03 habilitado",prefix);
		multibhoppre[id]=true;
	}
	else 
	{
		multibhoppre[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Multi Bhop Prestrafe:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public duck_show(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Ducks Prestrafe:^x03 habilitado",prefix);
		showduck[id]=true;
	}
	else 
	{
		showduck[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Ducks Prestrafe:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public lj_show(id)
{
	if(ljpre[id]==false) 
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 LJ Prestrafe:^x03 habilitado",prefix);
		ljpre[id]=true;
	}
	else 
	{
		ljpre[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 LJ Prestrafe:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public enable_sounds(id)
{
	if(uq_sounds)
	{
		...
		{
			ColorChat(id, BLUE, "^x04[%s]^x01 Sonidos:^x03 habilitado",prefix);
			enable_sound[id]=true;
		}
		else 
		{
			enable_sound[id]=false;
			ColorChat(id, RED, "^x04[%s]^x01 Sonidos:^x03 deshabilitado",prefix);
		}
	}
	else ColorChat(id, RED, "^x04[%s]^x01 Sonidos^x03 deshabilitados",prefix);
	return PLUGIN_HANDLED;
}
public ShowedgeFail(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Mostrar edge al fallar:^x03 habilitado",prefix);
		Show_edge_Fail[id]=true;
	}
	else 
	{
		Show_edge_Fail[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Mostrar edge al fallar:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public Showedge(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Mostrar edge:^x03 habilitado",prefix);
		Show_edge[id]=true;
	}
	else 
	{
		Show_edge[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Mostrar edge:^x03 deshabilitado",prefix);
	}
	return PLUGIN_HANDLED;
}
public heightshow(id)
{
	...
	{
		ColorChat(id, BLUE, "^x04[%s]^x01 Mostrar distancias de caida/altura:^x03 habilitado",prefix);
		height_show[id]=true;
	}
	else 
	{
		height_show[id]=false;
		ColorChat(id, RED, "^x04[%s]^x01 Mostrar distancias de caida/altura:^x03 deshabilitado",prefix);
	}
...
public ingame_st_stats(id)
{
	if(uq_istrafe)
	{
		if(kz_beam[id])
		{
			ColorChat(id, RED, "^x04[%s]^x01 La estadistica de strafe^x03 no funciona^x01 con /ljbeam",prefix);
		}
		else
		{
			if(ingame_strafe[id]==false) 
			{
				ColorChat(id, BLUE, "^x04[%s]^x01 Mostrar estadisticas de strafe:^x03 habilitado",prefix);
				ingame_strafe[id]=true;
			}
			else 
			{
				ingame_strafe[id]=false;
				ColorChat(id, RED, "^x04[%s]^x01 Mostrar estadisticas de strafe:^x03 deshabilitado",prefix);
			}
		}
	}
	else ColorChat(id, RED, "^x04[%s]^x03 Mostrar estadisticas de strafe deshabilitado",prefix);
...
public ResetHUD(id)
{
	if(is_user_alive(id) && !is_user_bot(id) && !is_user_hltv(id) )
	{
		...
		if(firstshow[id]==false)
		{
			if( uq_airaccel )
			{
				ColorChat(id, RED, "^x01[%s]^x01 Server runs:^x03 100aa.^x01 Type /ljtop to show Tops Menu. Type /ljsmenu to show Options Menu",prefix);
			}
			else 
			{
				ColorChat(id, BLUE, "^x01[%s]^x01 Server runs:^x03 10aa.^x01^x01 Type /ljtop to show Tops Menu. Type /ljsmenu to show Options Menu ",prefix);		
			}
			firstshow[id]=true;
...
public FwdPlayerSpawn(id)
{
	...
	{
		/*if( !task_exists(id+434490, 0) )
			set_task(0.1, "tskFps", id+434490, "", 0, "b", 0);*/
...
{	
	...
	if(kz_sql == 0)
	{
		client_print(id,print_console,"[KZ] Se han reiniciado los Tops!");
		server_print("[KZ] Se han reiniciado los Tops!");
	...
	{
		client_print(id,print_console,"[KZ] You use SQL Version of Stats, delete yourself your database");
		server_print("[KZ] You use SQL Version of Stats, delete yourself your database!!!");
	}
...
{	
	...
	len = format(MenuBody, 511, "\yMenu 1/3 ^n");
	
	if(g_lj_stats[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r1. \yMostrar estadisticas: \wHabilitar");
		keys |= (1<<0);
	}
	else
	{
		keys |= (1<<0);
		len += format(MenuBody[len], 511-len, "^n\r1. \yMostrar estadisticas: \dDeshabilitar");
	}
	
	if(gHasColorChat[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r2. \yColorchat: \wHabilitar");
		keys |= (1<<1);
	}
	else
	{
		keys |= (1<<1);
		len += format(MenuBody[len], 511-len, "^n\r2. \yColorchat: \dDeshabilitar");
	}
	
	if(speedon[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r3. \yMostrar velocidad: \wHabilitar");
		keys |= (1<<2);
	}
	else
	{
		keys |= (1<<2);
		len += format(MenuBody[len], 511-len, "^n\r3. \yMostrar velocidad: \dDeshabilitar");
	}
	
	if(showpre[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r4. \yMostrar prestrafe: \wHabilitar");
		keys |= (1<<3);
	}
	else
	{
		keys |= (1<<3);
		len += format(MenuBody[len], 511-len, "^n\r4. \yMostrar prestrafe: \dDeshabilitar");
	}
	
	if(streifstat[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r5. \yMostrar estadisticas de strafes: \wHabilitar");
		keys |= (1<<4);
	}
	else
	{
		keys |= (1<<4);
		len += format(MenuBody[len], 511-len, "^n\r5. \yMostrar estadisticas de strafes: \dDeshabilitar");
	}
	
	if(kz_beam[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r6. \yMostrar rayo luego de saltar: \wHabilitar");
		keys |= (1<<5);
	}
	else
	{
		keys |= (1<<5);
		len += format(MenuBody[len], 511-len, "^n\r6. \yMostrar rayo luego de saltar: \dDeshabilitar");
	}
	
	if(showduck[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r7. \yMostrar ducks prestrafe: \wHabilitar");
		keys |= (1<<6);
	}
	else
	{
		keys |= (1<<6);
		len += format(MenuBody[len], 511-len, "^n\r7. \yMostrar ducks prestrafe: \dDeshabilitar");
	}
	if(failearly[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r8. \yAviso de bhop fallido: \wHabilitar");
		keys |= (1<<7);
	}
	else
	{
		keys |= (1<<7);
		len += format(MenuBody[len], 511-len, "^n\r8. \yAviso de bhop fallido: \dDeshabilitar");
	}

	len += format(MenuBody[len], 511-len, "^n^n\r9. \wSiguiente");
	keys |= (1<<8);
		
	len += format(MenuBody[len], 511-len, "^n^n\r0. \wSalir");
	keys |= (1<<9);
...
{	
	new MenuBody[512], len, keys;
	len = format(MenuBody, 511, "\yMenu 2/3 ^n");
	
	if(multibhoppre[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r1. \yMulti Bhop Prestrafe: \wHabilitar");
		keys |= (1<<0);
	}
	else
	{
		keys |= (1<<0);
		len += format(MenuBody[len], 511-len, "^n\r1. \yMulti Bhop Prestrafe: \dDeshabilitar");
	}
	if(Show_edge[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r2. \yMostrar Edge: \wHabilitar");
		keys |= (1<<1);
	}
	else
	{
		keys |= (1<<1);
		len += format(MenuBody[len], 511-len, "^n\r2. \yMostrar Edge: \dDeshabilitar");
	}
	if(Show_edge_Fail[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r3. \yMostrar edge al fallar \d(sin bloque)\y: \wHabilitar");
		keys |= (1<<2);
	}
	else
	{
		keys |= (1<<2);
		len += format(MenuBody[len], 511-len, "^n\r3. \yMostrar edge al fallar \d(sin bloque)\y: \dDeshabilitar");
	}
	
	len += format(MenuBody[len], 511-len, "^n\r4. \yBloques minimos por edge: \w%d",user_block[id][1]);
	keys |= (1<<3);
	len += format(MenuBody[len], 511-len, "^n\r5. \yBloques maximos por edge: \w%d",user_block[id][0]);
	keys |= (1<<4);
	len += format(MenuBody[len], 511-len, "^n\r6. \yPrestrafe minimos para mostrar: \w%d",min_prestrafe[id]);
	keys |= (1<<5);
	
	if(beam_type[id]==1)
	{
		len += format(MenuBody[len], 511-len, "^n\r7. \yTipo de rayo: \w1");
		keys |= (1<<6);
	}
	else if(beam_type[id]==2)
	{
		len += format(MenuBody[len], 511-len, "^n\r7. \yTipo de rayo: \w2");
		keys |= (1<<6);
	}
	
	
	len += format(MenuBody[len], 511-len, "^n^n\r8. \wAnterior");
	keys |= (1<<7);
	len += format(MenuBody[len], 511-len, "^n\r9. \wSiguiente");
	keys |= (1<<8);	
	len += format(MenuBody[len], 511-len, "^n^n\r0. \wSalir");
	keys |= (1<<9);
	...
	switch((key+1))
	{
		...
		case 5:
		{
			if(user_block[id][0]==uq_maxedge)
			{
				...
				client_print(id,print_center,"Alcanzaste un valor maximo (%d)",uq_maxedge);
			}
		...
		case 6:
		{
			...
			{
				min_prestrafe[id]=0;
				client_print(id,print_center,"Alcanzaste un valor maximo (320)");
			}
		...
		case 7:
		{
			...
			{
				beam_type[id]=2;
				client_print(id,print_center,"Rayo al mostrar strafes");
			}
			else
			{
				beam_type[id]=1;
				client_print(id,print_center,"Rayo por defecto");
			}
			Option2(id);	
		}
		case 8:
...
{	
	new MenuBody[512], len, keys;
	len = format(MenuBody, 511, "\yMenu 3/3 ^n");
	
	if(enable_sound[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r1. \ySonidos: \wHabilitar");
		keys |= (1<<0);
	}
	else
	{
		keys |= (1<<0);
		len += format(MenuBody[len], 511-len, "^n\r1. \ySonidos: \dDeshabilitar");
	}
	if(showjofon[id]==true)
	{
		len += format(MenuBody[len], 511-len, "^n\r2. \yMostrar JumpOff: \wHabilitar");
		keys |= (1<<1);
	}
	else
	{
		keys |= (1<<1);
		len += format(MenuBody[len], 511-len, "^n\r2. \yMostrar JumpOff: \dDeshabilitar");
	}
	if(height_show[id])
	{
		len += format(MenuBody[len], 511-len, "^n\r3. \yMostrar altura de salto: \wHabilitar");
		keys |= (1<<2);
	}
	else
	{
		len += format(MenuBody[len], 511-len, "^n\r3. \yMostrar altura de salto: \dDeshabilitar");
		keys |= (1<<2);
	}
	if(jofon[id])
	{
		len += format(MenuBody[len], 511-len, "^n\r4. \yIndicador de jumpoff: \wHabilitar");
		keys |= (1<<3);
	}
	else
	{
		len += format(MenuBody[len], 511-len, "^n\r4. \yIndicador de jumpoff: \dDeshabilitar");
		keys |= (1<<3);
	}
	if(jheight_show[id])
	{
		len += format(MenuBody[len], 511-len, "^n\r5. \yAltura de salto: \wHabilitar");
		keys |= (1<<4);
	}
	else
	{
		len += format(MenuBody[len], 511-len, "^n\r5. \yAltura de salto: \dDeshabilitar");
		keys |= (1<<4);
	}
	if(uq_istrafe)
	{
		if(ingame_strafe[id])
		{
			len += format(MenuBody[len], 511-len, "^n\r6. \yEstadisticas de strafe: \wHabilitar");
			keys |= (1<<5);
		}
		else
		{
			len += format(MenuBody[len], 511-len, "^n\r6. \yEstadisticas de strafe: \dDeshabilitar");
			keys |= (1<<5);
		}
	}
	else len += format(MenuBody[len], 511-len, "^n\r6. \dEstadisticas de strafe deshabilitado");
	
	len += format(MenuBody[len], 511-len, "^n\r7. \yTiempo de mostrar las estadisticas de strafe: \w%ds",showtime_st_stats[id]/10);
	keys |= (1<<6);
		
	len += format(MenuBody[len], 511-len, "^n^n\r9. \wAnterior");
	keys |= (1<<8);
...
public OptionMenu3(id, key)
{
	switch((key+1))
	{
		...
		case 7:
		{
			...
			{
				client_print(id,print_center,"Maximo 20 segundos");
