...
<html>
	...
	<body>
		<?php
			$dataDone = 'achievement_count';

			$dataDoneText = 'Logros desbloqueados';
			...
			echo "<tr align='center' bgcolor='#52697B'>
					...
					<th width='25%' align='center'>".$dataDoneText."</th>
				</tr>";
			
			if($topOk == '1') {
				$topQuery = "SELECT COUNT(id) AS rank
						FROM td_users u
						WHERE (u.".$dataDone." > (SELECT ".$dataDone." FROM td_users u2 WHERE u2.id = '".$topId."') OR (u.".$dataDone." = (SELECT ".$dataDone." FROM td_users u2 WHERE u2.id = '".$topId."') AND u.id <= '".$topId."'));";
				
				$topData = mysql_fetch_array(mysql_query($topQuery));
				$userRank = $topData[0];
				
				$topQuery = "SELECT id, name, ".$dataDone."
						FROM td_users
						WHERE id = '".$topId."'";
